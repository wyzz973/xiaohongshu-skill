...
Agent 根据以下因素自主选择发布类型，不需要人工指定。

### 4 种技术类型对比

| 类型 | 命令 | 排版能力 | 图片 | 最适场景 |
|------|------|---------|------|---------|
| **图文** | `fill-publish` | 纯文本 | 用户提供 1+ 张 | 教程截图、对比图、清单配图 |
| **长文** | `long-article --markdown` | Markdown（标题/列表/引用）+ 模板 | 可选 | 深度分析、对比测评、800+ 字结构化内容 |
| **文字配图** | `publish-text2image` | 纯文本 | 平台自动生成 | 快速观点、短内容、痛点共鸣 |
| **视频** | `fill-publish-video` | 纯文本描述 | 无（视频） | 演示录屏、操作教程 |
...
2. 内容 > 800 字 且 有结构（多段标题/列表/引用）？
   → 是：长文（Markdown 排版优势明显，模板美观）
...
- 读 `evolution.json`，查各类型 winning_patterns 的权重分布
- 查近 14 天各类型的平均 CTR 和互动率，优先用表现好的类型
- 避免连续 3 篇同一类型，保持内容多样性

### 决策记录

在 `drafts/` 的 metadata 中记录：
```json
{
  "type": "long_article",
  "type_reason": "内容 1200 字+有 3 个分段标题，长文 Markdown 排版效果最好"
}
```
