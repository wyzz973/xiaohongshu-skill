    ...
    def wait_for_load(self, timeout: float = 60.0) -> None:
        ...
        while time.monotonic() < deadline:
            try:
                ...
                logger.debug("轮询 readyState 异常: %s", e)
            time.sleep(0.5)
...
(() => {
    ...
    if (!html) return '';
    let h = 5381;
    const len = Math.min(html.length, 50000);
    for (let i = 0; i < len; i++) {
        h = ((h << 5) + h + html.charCodeAt(i)) & 0x7fffffff;
    }
...
"""
    ...
    def wait_dom_stable(self, timeout: float = 10.0, interval: float = 0.5) -> None:
        """等待 DOM 稳定（连续两次 DOM hash+length 一致）。"""
        last_hash = ""
        deadline = time.monotonic() + timeout
        while time.monotonic() < deadline:
            try:
                hash_str = self.evaluate(self._DOM_HASH_JS)
                if hash_str and hash_str == last_hash:
                    return
                last_hash = hash_str or ""
            except CDPError as e:
                logger.debug("DOM 稳定检测异常: %s", e)
            time.sleep(interval)
    ...
    def input_content_editable(self, selector: str, text: str) -> None:
        ...
        # 非 ASCII 字符（中文等）用 Input.insertText 以正确触发
        # React/Vue 合成事件，ASCII 字符用 keyDown/keyUp。
        for char in text:
            if char == "\n":
    ...
    def type_text(self, text: str, delay_ms: int = 50) -> None:
        ...
        ASCII 字符用 keyDown/keyUp 事件，非 ASCII（中文等）用
        Input.insertText 以正确触发 React/Vue 合成事件。
        """
        for char in text:
            if ord(char) < 128:
                self._send_session(
