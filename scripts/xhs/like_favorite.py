...
def _prepare_page(
    page: Page, feed_id: str, xsec_token: str, xsec_source: str = "pc_feed"
) -> None:
    """导航到 feed 详情页。"""
...
def _toggle_like(page: Page, feed_id: str, target_liked: bool) -> ActionResult:
    ...
    # 重试前重新检查状态（避免误撤销已成功的操作）
    try:
        liked, _ = _get_interact_state(page, feed_id)
        if liked == target_liked:
            return ActionResult(
                feed_id=feed_id, success=True,
                message=f"{action_name}成功（延迟确认）",
            )
    except NoFeedDetailError:
        pass
    ...
    try:
        liked, _ = _get_interact_state(page, feed_id)
        ok = liked == target_liked
        return ActionResult(
            feed_id=feed_id, success=ok,
            message=f"{action_name}{'成功' if ok else '失败'}",
        )
    except NoFeedDetailError:
        return ActionResult(
            feed_id=feed_id, success=False,
            message=f"{action_name}状态未知",
        )
