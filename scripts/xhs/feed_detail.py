...
from .errors import CDPError, NoFeedDetailError, PageNotAccessibleError
from .human import (
...
def _extract_feed_detail(page: Page, feed_id: str) -> FeedDetailResponse:
    ...
    # 检测 SPA 重定向（xsec_token 过期时页面会静默跳回首页）
    current_url = page.evaluate("window.location.href") or ""
    if feed_id not in current_url:
        logger.warning(
            "页面被重定向（xsec_token 可能过期），当前 URL: %s", current_url
        )
        raise NoFeedDetailError()
